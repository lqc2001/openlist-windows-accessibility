...
- **纯净界面**：默认隐藏所有播放信息，提供真正的沉浸式观影体验

### 播放操作指南
1. **启动播放**：在文件列表中选择视频文件，按回车或双击
2. **播放控制**：使用空格键播放/暂停，ESC键退出
3. **进度控制**：左右箭头快进/快退，上下箭头调节音量
4. **全屏切换**：按F11键在全屏和窗口模式间切换
5. **信息显示**：通过菜单显示/隐藏播放信息（标题、时间进度、控制提示等）

### 菜单系统
- **Alt+P**：显示/隐藏完整菜单系统
- **播放信息控制**：显示/隐藏播放信息（标题、时间进度、控制提示等）
- **音轨选择**：通过菜单切换多语言音轨
- **播放控制**：完整的播放和设置选项
...
### v1.2.1 (2025-11-20) - 音轨检测修复 🎛️
- 🎵 **音轨重复问题修复**：解决视频播放时显示多余"音轨1"的问题，确保音轨列表准确
- 🔍 **智能音轨检测**：改进VLC音轨检测逻辑，优先使用真实音轨描述，避免重复创建
- ⏰ **分阶段自动检测**：实现2000ms和4000ms分阶段检测，确保VLC完全解析音轨信息
- 🎯 **问题根源解决**：修复VLC track_count与实际有效音轨数量不匹配导致的重复问题
- 📝 **详细日志跟踪**：增加详细的音轨处理日志，便于问题诊断和调试
- 🚀 **用户体验提升**：视频播放时音轨自动显示，无需手动刷新菜单
- 🛠️ **技术架构优化**：改进音轨处理优先级：真实描述 > 实际音轨 > VLC计数
- 📚 **故障排除指南**：更新音轨问题排查方法和解决方案

### v1.2.0 (2025-11-18) - 视频播放信息隐藏功能 🎭
- 🎬 **沉浸式观影体验**：默认隐藏所有播放信息，提供完全纯净的视频观看界面
- 🎯 **智能信息控制**：通过菜单显示/隐藏播放信息（标题、时间进度、控制提示等）
- 🖥️ **完整界面控制**：包括视频标题、时间显示、控制提示的统一管理
- 📱 **动态菜单更新**：菜单项文本根据当前状态自动更新（显示/隐藏）
- 🎮 **优化全屏快捷键**：全屏切换快捷键从F键改为F11键，符合用户习惯
- 🎨 **用户界面简化**：移除不必要的快捷键显示，界面更加简洁直观
- 📚 **帮助文档更新**：完整更新控制说明和帮助信息，反映新的功能设计
- 🔧 **技术架构优化**：统一的信息控制机制，确保界面响应和状态同步
